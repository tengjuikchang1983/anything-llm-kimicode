...
async function getCustomModels(provider = "", apiKey = null, basePath = null) {
  ...
  switch (provider) {
    ...
    case "kimicode":
      return await getKimiCodeModels(apiKey);
    case "foundry":
...
async function getKimiCodeModels(_apiKey = null) {
  const apiKey =
    _apiKey === true
      ? process.env.KIMI_CODE_API_KEY
      : _apiKey || process.env.KIMI_CODE_API_KEY || null;
  ...
  const models = await openai.models
    ...
    .then((results) => results.data)
    .catch((e) => {
      console.error(`KimiCode:listModels`, e.message);
      return [];
    });

  // Api Key was successful so lets save it for future uses
  if (models.length > 0) process.env.KIMI_CODE_API_KEY = apiKey;
  return { models, error: null };
}
